...
export class AdminController {
  ...
  // Bracket map: source round → { nextRound, slot }
  private static readonly BRACKET_MAP: Record<number, { nextRound: number; slot: 'home' | 'away' }> = {
  ...
  private static async propagateBracketWinner(matchId: string): Promise<void> {
    const match = await prisma.match.findUnique({
      where: { id: matchId },
      select: { round: true, homeTeamId: true, awayTeamId: true, homeScore: true, awayScore: true, homePenalties: true, awayPenalties: true, status: true },
    });

    if (!match || match.status !== MatchStatus.FINISHED || match.round == null) return;

    const entry = AdminController.BRACKET_MAP[match.round];
    if (!entry) return;
  ...
  static async updateMatch(req: Request, res: Response): Promise<void> {
    ...
    if (match.status === MatchStatus.FINISHED) {
      await AdminController.propagateBracketWinner(match.id);
    }
